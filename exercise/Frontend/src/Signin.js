...
class Signin extends Component {
    ...
    locker(event) {
        event.persist();
        if (this.state.locker[event.target.id - 1].state === 1) {
            console.log("Sigin" + event.target.id)
            this.props.setPage(7, event.target.id)
        }
    };
    render() {
            ...
            return (
                <div>
                    ...
                    <div className="mt-2 rounded row justify-content-center text-center">
                        <div className=" bg-secondary rounded mr-2 col-1" ></div><h5>ไม่ถูกใช้งาน</h5>
                        <div className=" bg-danger rounded ml-5 mr-2 col-1" ></div><h5>ถูกใช้งาน</h5>
                    </div>
